...
int server(int connect_port)
{
	...
	for (;;)
	{
		...
		std::cout << "last error before accepting socket: " << error << "\n";
		client_socket = accept(mysocket,
			reinterpret_cast<sockaddr*> (&client_addr),
			&client_addr_size);
		error = WSAGetLastError();
		std::cout << "last error after accepting socket: " << error << "\n";
		...
		DWORD thID;
		error = WSAGetLastError();
		std::cout << "last error before create thread: " << error << "\n";
		CreateThread(NULL, NULL, client_service, &client_socket, NULL, &thID);
		error = WSAGetLastError();
		std::cout << "last error in the end of accepting connection: " << error << "\n";
	}
	closesocket(mysocket);
...
{
	int error = WSAGetLastError();
	std::cout << "last error in client service: " << error << "\n";
	...
	error = WSAGetLastError();
	std::cout << "last error in creating socket in client service: " << error << "\n";
	int bytes_recv = 0;
	while (true) {
		try {
			if (bytes_recv = recv(my_sock, &packet_buff, sizeof(packet_buff), 1) < 0)
			{
				error = WSAGetLastError();
				std::cout << "last error recv in client service: " << error << "\n";
				EnterCriticalSection(&section1);
				if (userWeather.size() != 0)	userWeather.remove(my_sock);
				LeaveCriticalSection(&section1);
				EnterCriticalSection(&section2);
				if (userShares.size() != 0)	userShares.remove(my_sock);
				LeaveCriticalSection(&section2);
				EnterCriticalSection(&section3);
				if (userExchange.size() != 0)	userExchange.remove(my_sock);
				LeaveCriticalSection(&section3);
				return 0;
			}
			error = WSAGetLastError();
			std::cout << "last error in client service after critical section: " << error << "\n";
			std::cout << "data from client: \n" << packet_buff << std::endl;
			if (packet_buff == 'w')
			{
				EnterCriticalSection(&section1);
				if (!subscribe[0]) {
